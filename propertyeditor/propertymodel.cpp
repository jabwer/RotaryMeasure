...
void PropertyModel::refreshProperties()
{
	...
	if (m_object && m_plugins)
		for (int i = 0;i < m_object->metaObject()->propertyCount();i++) {
			if (m_object->metaObject()->property(i).isDesignable() && m_object->metaObject()->property(i).isValid() && m_object->metaObject()->property(i).isWritable() && m_object->metaObject()->property(i).isReadable() && QString(m_object->metaObject()->property(i).name()) != "objectName") {
...
PropertyInterface* PropertyModel::createPropery(QObject* object, int property) const
{
	...
	if (object->metaObject()->property(property).isValid() && object->metaObject()->property(property).isWritable() && object->metaObject()->property(property).isReadable() && QString(object->metaObject()->property(property).name()) != "objectName")
		foreach(PropertyInterface* plugin, (*m_plugins))
